...
static void check_CFI_establish(CFI_cdesc_t *dv, void *base_addr,
  ...
  // Checking failure/s.cppess .cppording to combination of args forbidden by the
  // standard:
  int numErr{0};
...
// Helper to get size without making internal compiler functions .cppessible
...
static void check_CFI_allocate(CFI_cdesc_t *dv,
  ...
  // S.cppess/failure .cppording to standard
  int numErr{0};
  int expectedRetCode{CFI_SUCCESS};
  if (rank > CFI_MAX_RANK) {
    ++numErr;
