...
subroutine s4()
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: concurrent-header expression references variable 'i' in LOCAL locality-spec
...
subroutine s13()
  ...
  integer :: ivar
  associate (avar => ivar)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: concurrent-header expression references variable 'ivar' in LOCAL locality-spec
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: concurrent-header expression references variable 'mvar' in LOCAL locality-spec
