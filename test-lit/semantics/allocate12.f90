...
! is a coarray.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray specification must not appear in ALLOCATE when allocatable object is not a coarray
  allocate(x1[*])
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray specification must not appear in ALLOCATE when allocatable object is not a coarray
  allocate(x2(10)[*])
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray specification must appear in ALLOCATE when allocatable object is a coarray
  allocate(cx1)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray specification must appear in ALLOCATE when allocatable object is a coarray
  allocate(cx2(10))
...
! than the corank of the allocate-object.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Corank of coarray specification in ALLOCATE must match corank of alloctable coarray
...
! C950: An allocate-object shall not be a coindexed object.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Allocatable object must not be coindexed in ALLOCATE
