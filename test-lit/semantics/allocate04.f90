...
! only testing unlimited polymorphic and abstract-type here
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is unlimited polymorphic
  allocate(u1)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is unlimited polymorphic
  allocate(u2(2))
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is unlimited polymorphic
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is of abstract type
  allocate(p1)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is of abstract type
  allocate(p2(2))
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Either type-spec or source-expr must appear in ALLOCATE when allocatable object is of abstract type
