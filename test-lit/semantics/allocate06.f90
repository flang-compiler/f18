...
! is assumed.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type parameters in type-spec must be assumed if and only if they are assumed for allocatable object in ALLOCATE
  allocate(character(len=5):: ac1)
  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: Type parameters in type-spec must be assumed if and only if they are assumed for allocatable object in ALLOCATE
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type parameters in type-spec must be assumed if and only if they are assumed for allocatable object in ALLOCATE
