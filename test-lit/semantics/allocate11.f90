...
! Type-spec shall not specify a type that has a coarray ultimate component.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-spec in ALLOCATE must not specify a type with a coarray ultimate component
  allocate(A:: var)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-spec in ALLOCATE must not specify a type with a coarray ultimate component
  allocate(B:: var)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-spec in ALLOCATE must not specify a type with a coarray ultimate component
...
! unlimited polymorphic or a type that extends TEAM_TYPE.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-Spec in ALLOCATE must not be TEAM_TYPE from ISO_FORTRAN_ENV when an allocatable object is a coarray
  allocate(TEAM_TYPE:: var[5:*])
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-Spec in ALLOCATE must not be C_PTR or C_FUNPTR from ISO_C_BINDING when an allocatable object is a coarray
...
!  ! Valid constructs
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: SOURCE or MOLD expression type must not be TEAM_TYPE from ISO_FORTRAN_ENV when an allocatable object is a coarray
  allocate(var[5:*], SOURCE=my_team)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: SOURCE or MOLD expression type must not be C_PTR or C_FUNPTR from ISO_C_BINDING when an allocatable object is a coarray
