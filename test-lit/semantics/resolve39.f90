...
subroutine s1
  implicit none
  real(8) :: x = 2.0
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: The associate name 'a' is already used in this associate statement
  associate(a => x, b => x+1, a => x+2)
    x = b
  end associate
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: No explicit type declared for 'b'
  x = b
end

subroutine s2
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Associate name 'a' must have a type
  associate (a => z'1')
  end associate
end
...
! mis-parsed function references into array references
  ...
  associate (c => a(2:10:2))
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Reference to array 'c' with empty subscript list
    print *, c()
  end associate
end
