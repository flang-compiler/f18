...
module m01
 ...
 contains
  ...
  subroutine test01(x) ! 15.5.2.4(2)
    class(t), intent(in) :: x[*]
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coindexed polymorphic object may not be associated with a polymorphic dummy argument 'x='
    call poly(x[1])
  end subroutine
  ...
  subroutine test02(x) ! 15.5.2.4(2)
    class(t), intent(in) :: x(*)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Assumed-size polymorphic array may not be associated with a monomorphic dummy argument 'x='
    call mono(x)
  end subroutine
  ...
  subroutine test03 ! 15.5.2.4(2)
    type(pdt(0)) :: x
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with TYPE(*) dummy argument 'x=' may not have a parameterized derived type
    call typestar(x)
  end subroutine

  subroutine test04 ! 15.5.2.4(2)
    type(tbp) :: x
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with TYPE(*) dummy argument 'x=' may not have type-bound procedure 'binding'
    call typestar(x)
  end subroutine

  subroutine test05 ! 15.5.2.4(2)
    type(final) :: x
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with TYPE(*) dummy argument 'x=' may not have FINAL subroutine 'subr02'
    call typestar(x)
  end subroutine
  ...
  subroutine test07(x) ! 15.5.2.4(6)
    type(alloc) :: x[*]
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coindexed actual argument with ALLOCATABLE ultimate component '%a' must be associated with a dummy argument 'x=' with VALUE or INTENT(IN) attributes
    call out01(x[1])
  end subroutine

  subroutine test08(x) ! 15.5.2.4(13)
    real :: x(1)[*]
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coindexed scalar actual argument must be associated with a scalar dummy argument 'x='
  ...
  subroutine test09(ashape, polyarray, c) ! 15.5.2.4(14), 15.5.2.11
    ...
    class(t) :: polyarray(*)
    character(10) :: c(:)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Whole scalar actual argument may not be associated with a dummy argument 'x=' array
    call assumedsize(x)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Scalar POINTER target may not be associated with a dummy argument 'x=' array
    call assumedsize(p(1))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Element of assumed-shape array may not be associated with a dummy argument 'x=' array
    call assumedsize(ashape(1))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Polymorphic scalar may not be associated with a dummy argument 'x=' array
  ...
  subroutine test10(a) ! 15.5.2.4(16)
    ...
    real :: a(*)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Scalar actual argument may not be associated with assumed-shape dummy argument 'x='
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Assumed-size array may not be associated with assumed-shape dummy argument 'x='
    call assumedshape(a)
  end subroutine
  ...
  subroutine test12 ! 15.5.2.4(21)
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with ASYNCHRONOUS dummy argument 'x=' must be definable
    call asynchronous(a(j))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with VOLATILE dummy argument 'x=' must be definable
    call volatile(a(j))
  end subroutine
  ...
  subroutine test13(a, b) ! 15.5.2.4(22)
    ...
    call coarr(a)  ! ok
    call volcoarr(b)  ! ok
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: VOLATILE attribute must match for dummy argument 'x=' when actual argument has a coarray ultimate component '%a'
    call coarr(b)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: VOLATILE attribute must match for dummy argument 'x=' when actual argument has a coarray ultimate component '%a'
    call volcoarr(a)
  end subroutine
  ...
  subroutine test14(a,b,c,d) ! C1538
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coindexed ASYNCHRONOUS or VOLATILE actual argument may not be associated with dummy argument 'x=' with ASYNCHRONOUS or VOLATILE attributes unless VALUE
  ...
  subroutine test15() ! C1539
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: ASYNCHRONOUS or VOLATILE actual argument that is not simply contiguous may not be associated with a contiguous dummy argument 'x='
