...
subroutine s1
  ...
  z = f(1.0)
  z = f(1.0, 2.0)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: No specific procedure of generic 'f' matches the actual arguments
  z = f(1.0, 2.0, 3.0)
end
...
subroutine s3
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: No intrinsic or user-defined .FOO. matches operand types LOGICAL(4) and LOGICAL(4)
  a = b .foo. c
end
