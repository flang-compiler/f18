...
contains
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 's' may not be the name of both a generic interface and a procedure unless it is a specific procedure of the generic
  subroutine s
  end subroutine
end module
...
subroutine s6
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Generic 'g' may not have specific procedures 'sa' and 'sb' as their interfaces are not distinguishable
  use m6a, g => gg
  use m6b, g => gg
end
...
subroutine s8
  ...
  use m8b
  use m8c
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Reference to 'g' is ambiguous
  g = 1
end
...
subroutine s9c
  use m9a
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Generic interface 'g' has ambiguous specific procedures from modules 'm9a' and 'm9c'
  use m9c
end
