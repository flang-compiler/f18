...
subroutine s1()
  ...
  associate (avar => ivar)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: DO CONCURRENT step expression should not be zero
    do concurrent (i = 1:2:0) default(none) shared(jvar) local(kvar)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'ivar' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
      ivar =  &
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'ivar' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
        ivar + i
      block
        real :: bvar
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'avar' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
        avar = 4
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'x' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
        x = 3.5
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'mvar' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
      mvar = 3.5
    end do
  end associate
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Variable 'a' from an enclosing scope referenced in DO CONCURRENT with DEFAULT(NONE) must appear in a locality-spec
      a%x = 3.5
    end do
  end select
