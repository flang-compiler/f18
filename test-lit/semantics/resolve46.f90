...
program main
  ...
  intrinsic :: amin0
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'haltandcatchfire' is not a known intrinsic procedure
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Procedure pointer 'p' associated with incompatible procedure designator 'amin0'
  p => amin0
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Procedure pointer 'p' associated with incompatible procedure designator 'amin1'
  p => amin1
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'bessel_j0' is not a specific intrinsic procedure
  p => bessel_j0
end program main
