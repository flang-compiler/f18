...
module m
 contains
  ...
  subroutine test1 ! 15.5.2.9(5)
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument procedure has interface incompatible with dummy argument 'p='
    call s01(intfunc)
    call s01(p) ! ok
    call s01(procptr()) ! ok
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument procedure has interface incompatible with dummy argument 'p='
    ...
    call s01(null(ip))
    call s01(sin) ! ok
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure pointer dummy argument 'p=' must be a POINTER unless INTENT(IN)
    call s02(realfunc)
    call s02(p) ! ok
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument procedure has interface incompatible with dummy argument 'p='
    call s02(ip)
  ...
  subroutine test2(unknown,ds,drf,dif) ! 15.5.2.9(2,3)
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure dummy argument 's=' is a function but must be a subroutine
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure dummy argument 's=' is a function but must be a subroutine
    call callsub(pif)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure dummy argument 'f=' is a subroutine but must be a function
    call takesrealfunc1(callsub)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure dummy argument 'f=' is a subroutine but must be a function
    call takesrealfunc1(ds)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument associated with procedure dummy argument 'f=' is a subroutine but must be a function
    call takesrealfunc1(ps)
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument function associated with procedure dummy argument 'f=' has incompatible result type
