...
! RUN: not %flang -fdebug-resolve-names -fparse-only %s 2>&1 | FileCheck --check-prefixes=WARNING,ERROR --implicit-check-not error: %s


complex, parameter :: okj1 = 127_1, okz1 = (+127_1, -128_1)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Integer literal is too large for INTEGER(KIND=1)
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Integer literal is too large for INTEGER(KIND=2)
...
!WARNING: [[@LINE+1]]:{{[0-9]+}}:{{.*}} Integer literal is too large for default INTEGER(KIND=4); assuming INTEGER(KIND=8)
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Integer literal is too large for INTEGER(KIND=8)
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Integer literal is too large for INTEGER(KIND=16)
