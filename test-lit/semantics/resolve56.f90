...
! RUN: not %flang -fdebug-resolve-names -fparse-only %s 2>&1 | FileCheck --check-prefixes=ERROR --implicit-check-not error: %s
  ...
  select type(a)
    ...
    type is (real)
     res = acos(a)
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument for 'x=' has bad type 'CLASS(*)'
     res = acos(b)
  end select
  ...
  select type(a)
    type is (integer)
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument for 'x=' has bad type 'INTEGER(4)'
     res = acos(a)
  end select
  ...
  select type(b)
    type is (integer)
      ...
      ires = selected_int_kind(b)
      !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: No explicit type declared for 'c'
      ires = selected_int_kind(c)
      !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument for 'x=' has bad type 'CLASS(*)'
      res = acos(a)
    class default
      !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument for 'r=' has bad type 'CLASS(*)'
      ires = selected_int_kind(b)
  end select
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Actual argument for 'r=' has bad type 'CLASS(*)'
  ires = selected_int_kind(a)
