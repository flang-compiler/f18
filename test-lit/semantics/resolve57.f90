...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'prot' may not appear in a locality-spec because it is not definable
  do concurrent (i=1:5) local(prot)
  end do
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'a' may not appear in a locality-spec because it is not definable
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'var' may not appear in a locality-spec because it is not definable
  do concurrent (i=1:5) local(var)
  end do
end subroutine s7
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'iconst' may not appear in a locality-spec because it is not definable
