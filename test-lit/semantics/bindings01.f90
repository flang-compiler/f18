...
module m
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An ABSTRACT derived type must be extensible
  ...
  type, abstract :: abstract
   contains
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type-bound procedure 'ab3' may not be both DEFERRED and NON_OVERRIDABLE
  ...
  type, extends(nonoverride2) :: nonoverride3
   contains
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Override of NON_OVERRIDABLE 'no1' is not permitted
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Non-ABSTRACT extension of ABSTRACT derived type 'missing' lacks a binding for DEFERRED procedure 'am1'
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: The parent type is not extensible
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Derived type 'real' not found
  type, extends(real) :: badExtends3
  end type
  type :: base
    real :: component
   contains
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Procedure bound to non-ABSTRACT derived type 'base' may not be DEFERRED
  ...
  type, extends(base) :: extension
   contains
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: A type-bound procedure binding may not have the same name as a parent component
  ...
  type, extends(nopassBase) :: passExtends
   contains
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: A passed-argument type-bound procedure may not override a NOPASS procedure
    procedure :: tbp => s5
  end type
  type :: passBase
   contains
  ...
  type, extends(passBase) :: nopassExtends
   contains
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: A NOPASS type-bound procedure may not override a passed-argument procedure
    procedure, nopass :: tbp => s1
  end type
 contains
  subroutine s1(x)
