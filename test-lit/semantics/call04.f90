...
module m
 ...
 contains
  ...
  subroutine s01b ! C846 - can only be caught at a call via explicit interface
    !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: ALLOCATABLE coarray 'coarray' may not be associated with INTENT(OUT) dummy argument 'x='
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: ALLOCATABLE dummy argument 'x=' has corank 0 but actual argument has corank 1
    call s01a(coarray)
  end subroutine

  subroutine s02(x) ! C846
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An INTENT(OUT) dummy argument may not be, or contain, an ALLOCATABLE coarray
...
subroutine s06(x) ! C847
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An INTENT(OUT) dummy argument may not be, or contain, EVENT_TYPE or LOCK_TYPE
