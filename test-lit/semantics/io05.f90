! RUN: not %flang -fdebug-resolve-names -fparse-only %s 2>&1 | FileCheck --check-prefixes=ERROR --implicit-check-not error: %s
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: INQUIRE statement must have a UNIT number or FILE specifier
  inquire(err=9)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If FILE appears, UNIT must not appear
  inquire(10, file='abc', blank=c(22), iostat=stat8)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Duplicate FILE specifier
  inquire(file='abc', file='xyz')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Duplicate FORM specifier
  inquire(form=c(1), iostat=stat1, form=c(2), file='abc')

  !ERROR: [[@LINE+3]]:{{[0-9]+}}:{{.*}}error: Duplicate SIGN specifier
  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: Duplicate READ specifier
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Duplicate WRITE specifier
  inquire(1, read=c(1), write=c(2), sign=c(3), sign=c(4), read=c(5), write=c(1))

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Duplicate IOMSG specifier
  inquire(10, iomsg=msg, pos=ipos, iomsg=msg)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If ID appears, PENDING must also appear
