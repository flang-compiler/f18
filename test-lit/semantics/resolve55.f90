...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: ALLOCATABLE variable 'k' not allowed in a locality-spec
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: OPTIONAL argument 'arg' not allowed in a locality-spec
...
! Cannot have a dummy INTENT(IN) in a locality spec
  real, intent(in) :: arg
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: INTENT IN argument 'arg' not allowed in a locality-spec
  do concurrent(i=1:5) local(arg)
  end do
end subroutine s4
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Finalizable variable 'var' not allowed in a locality-spec
...
! This is not allowed
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Nonpointer polymorphic argument 'y' not allowed in a locality-spec
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray 'coarray_var' not allowed in a locality-spec
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Assumed size array 'arg' not allowed in a locality-spec
  do concurrent(i=1:5) local(arg)
  end do
end subroutine s8
