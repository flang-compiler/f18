...
module module1
 ...
 contains
  ...
  subroutine errors
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type parameter 'j' may not appear as a component of a structure constructor
    call type1arg(type1(0)(j=1))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Component 'n' conflicts with another component earlier in this structure constructor
    call type1arg(type1(0)(1,n=2))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Value in structure constructor lacks a component name
    call type1arg(type1(0)(n=1,2))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Component 'n' conflicts with another component earlier in this structure constructor
    call type1arg(type1(0)(n=1,n=2))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Unexpected value in structure constructor
    call type1arg(type1(0)(1,2))
    call type2arg(type2(0,0)(n=1,m=2))
    call type2arg(type2(0,0)(m=2))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Structure constructor lacks a value for component 'm'
    call type2arg(type2(0,0)())
    call type2arg(type2(0,0)(type1=type1(0)(n=1),m=2))
    call type2arg(type2(0,0)(type1=type1(0)(),m=2))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Component 'type1' conflicts with another component earlier in this structure constructor
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Type parameter 'k' may not appear as a component of a structure constructor
      k=2,m=3))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: ABSTRACT derived type 'abstract' may not be used in a structure constructor
