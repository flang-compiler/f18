...
subroutine s2
  ...
  real :: y[10,*], y2[*], x[*]
  ! C1113
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Selector 'y' was already used as a selector or coarray in this statement
  change team(t, x[10,*] => y, x2[*] => y)
  end team
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Selector 'x' was already used as a selector or coarray in this statement
  change team(t, x[10,*] => y, x2[*] => x)
  end team
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Coarray 'y' was already used as a selector or coarray in this statement
...
subroutine s3
  ...
  real :: y[10,*]
  ! C1114
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Team value must be of type TEAM_TYPE from module ISO_FORTRAN_ENV
