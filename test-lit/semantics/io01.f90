...
3 continue
  ...
  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: OPEN statement must have a UNIT or NEWUNIT specifier
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If ACCESS='DIRECT' appears, RECL must also appear
  open(access='direct')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If STATUS='STREAM' appears, RECL must not appear
  open(10, access='st'//'ream', recl=13)

  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: Duplicate NEWUNIT specifier
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If NEWUNIT appears, FILE or STATUS must also appear
  ...
  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: Duplicate UNIT specifier
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If ACCESS='DIRECT' appears, RECL must also appear
  open(access='dir'//'ect', 9, 9) ! nonstandard

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Duplicate ROUND specifier
  open(105, round=round_(1), pad='no', round='nearest')

  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: If NEWUNIT appears, UNIT must not appear
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If NEWUNIT appears, FILE or STATUS must also appear
  open(106, newunit=n)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: RECL value (-30) must be positive
  open(107, recl=40-70)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: RECL value (-36) must be positive
  open(108, recl=-  -  (-36)) ! nonstandard

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Invalid ACTION value 'reedwrite'
  open(109, access=Access, action='reedwrite', recl=77)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Invalid ACTION value 'nonsense'
  open(110, action=''//'non'//'sense', recl=77)

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Invalid STATUS value 'cold'
  open(111, status='cold')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Invalid STATUS value 'Keep'
  open(112, status='Keep')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If STATUS='NEW' appears, FILE must also appear
  open(113, status='new')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If STATUS='REPLACE' appears, FILE must also appear
  open(114, status='replace')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If STATUS='SCRATCH' appears, FILE must not appear
  open(115, file='abc', status='scratch')

  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If NEWUNIT appears, FILE or STATUS='SCRATCH' must also appear
