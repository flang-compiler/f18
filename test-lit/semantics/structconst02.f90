...
!    call scalararg(scalar(4)(5.,6,(7._8,8._2),4_'b',.true._4))
    ...
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Value in structure constructor of type CHARACTER(1) is incompatible with component 'ix' of type INTEGER(4)
    call scalararg(scalar(4)(ix='a'))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Value in structure constructor of type LOGICAL(4) is incompatible with component 'ix' of type INTEGER(4)
    call scalararg(scalar(4)(ix=.false.))
    !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Value in structure constructor of type INTEGER(4) is incompatible with component 'ix' of type INTEGER(4)
