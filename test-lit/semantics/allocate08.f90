...
!second part C945, specific to SOURCE, is not checked here.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Allocatable object in ALLOCATE must be type compatible with source expression from MOLD or SOURCE
...
program test_typeless
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Typeless item not allowed as SOURCE or MOLD in ALLOCATE
  allocate(x, mold=x'1')
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Typeless item not allowed as SOURCE or MOLD in ALLOCATE
  allocate(x, mold=sub)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Typeless item not allowed as SOURCE or MOLD in ALLOCATE
