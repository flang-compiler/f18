...
module m2
  use m1
  implicit none
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'foo' may not be the name of both a generic interface and a procedure unless it is a specific procedure of the generic
  interface foo
    module procedure s
  end interface
contains
...
subroutine foo
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Cannot use-associate 'foo'; it is already declared in this scope
  use m1
end

subroutine bar
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Cannot use-associate 'bar'; it is already declared in this scope
  use m1, bar => foo
end
