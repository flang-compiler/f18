...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: CYCLE must not leave a DO CONCURRENT statement
      cycle level0
    end do level1
  end do level0
end subroutine s4
...
subroutine s6()
  level0: do i = 1, 10
    level1: critical
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: CYCLE must not leave a CRITICAL statement
      cycle level0
    end critical level1
  end do level0
end subroutine s6
...
subroutine s7()
  level0: do i = 1, 10
    level1: critical
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: EXIT must not leave a CRITICAL statement
      exit level0
    end critical level1
  end do level0
end subroutine s7
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: CYCLE must not leave a CHANGE TEAM statement
      cycle level0
    end team level1
  end do level0
end subroutine s8
...
subroutine s9()
  ...
  level0: do i = 1, 10
    level1: change team(team_var)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: EXIT must not leave a CHANGE TEAM statement
      exit level0
    end team level1
  end do level0
end subroutine s9
