...
program main
  ...
  if (i .eq. 0) stop chr1
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: CHARACTER stop code must be of default kind
  if (i .eq. 0) stop chr2
  if (i .eq. 0) stop 1
  if (i .eq. 0) stop 1 + 2
  if (i .eq. 0) stop i
  ...
  if (i .eq. 0) stop 1 + i
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: INTEGER stop code must be of default kind
  if (i .eq. 0) stop invalid
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Stop code must be of INTEGER or CHARACTER type
