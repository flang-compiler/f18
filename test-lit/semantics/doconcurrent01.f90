...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An image control statement is not allowed in DO CONCURRENT
     SYNC ALL
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An image control statement is not allowed in DO CONCURRENT
     SYNC IMAGES (*)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An image control statement is not allowed in DO CONCURRENT
     SYNC MEMORY
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: RETURN is not allowed in DO CONCURRENT
     return
10 continue
...
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An image control statement is not allowed in DO CONCURRENT
      critical
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Call to an impure procedure is not allowed in DO CONCURRENT
        call ieee_get_status(status)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: IEEE_SET_HALTING_MODE is not allowed in DO CONCURRENT
        call ieee_set_halting_mode(flag, halting)
      end critical
    end team
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: ADVANCE specifier is not allowed in DO CONCURRENT
...
! Call to MOVE_ALLOC with non-coarray arguments in a DO CONCURRENT.  This is OK.
  ...
  do concurrent (i = 1:10)
!ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: Call to an impure procedure is not allowed in DO CONCURRENT
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: An image control statement is not allowed in DO CONCURRENT
    call move_alloc(ca, cb)
  end do
...
subroutine s7()
  ...
  do concurrent (i = 1:10)
!ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Call to an impure procedure component is not allowed in DO CONCURRENT
