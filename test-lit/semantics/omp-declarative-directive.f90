...
subroutine declare_simd_1(a, b)
  ...
  !ERROR: [[@LINE+3]]:{{[0-9]+}}:{{.*}}error: Internal: no symbol found for 'declare_simd_1'
...
subroutine declare_simd_2
  use m1
  procedure (sub) sub1
  !ERROR: [[@LINE+3]]:{{[0-9]+}}:{{.*}}error: Internal: no symbol found for 'sub1'
  !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: NOTINBRANCH and INBRANCH are mutually exclusive and may not appear on the same DECLARE SIMD directive
...
contains
  subroutine foo
    ...
    !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: MAP clause is not allowed on the DECLARE TARGET directive
