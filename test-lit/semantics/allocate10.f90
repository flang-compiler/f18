...
! statement and have the same rank as the allocate-object.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Arrays in ALLOCATE must have a shape specification or an expression of the same rank must appear in SOURCE or MOLD
  allocate(x1)
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Arrays in ALLOCATE must have a shape specification or an expression of the same rank must appear in SOURCE or MOLD
 ...
 !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: The number of shape specifications, when they appear, must match the rank of allocatable object
 allocate(x1(5, 5))
 !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: The number of shape specifications, when they appear, must match the rank of allocatable object
 ...
 !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: If SOURCE appears, the related expression must be scalar or have the same rank as each allocatable object in ALLOCATE
...
! If allocate-object is scalar, allocate-shape-spec-list shall not appear.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Shape specifications must not appear when allocatable object is scalar
  allocate(x(pos))
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Shape specifications must not appear when allocatable object is scalar
