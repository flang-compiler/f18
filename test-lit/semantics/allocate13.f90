...
! component of type EVENT_TYPE or LOCK_TYPE.
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: SOURCE expression type must not have potential subobject component of type EVENT_TYPE or LOCK_TYPE from ISO_FORTRAN_ENV
  ...
  !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: SOURCE expression type must not be EVENT_TYPE or LOCK_TYPE from ISO_FORTRAN_ENV
