...
1001 format(A)
     ...
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Format statement must be labeled
     format(A)
...
2054 format(3PI2)
     ...
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: Expected ',' or ')' in format expression
...
4001 format(2(X))
     ...
     !ERROR: [[@LINE+1]]:{{[0-9]+}}:{{.*}}error: 'DT' edit descriptor repeat specifier must be positive
4101 format(0(X), 0dt)

6001 format(((I0, B0)))

     !ERROR: [[@LINE+2]]:{{[0-9]+}}:{{.*}}error: 'A' edit descriptor 'w' value must be positive
