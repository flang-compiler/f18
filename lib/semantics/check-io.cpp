...
void IoChecker::Enter(const parser::ConnectSpec::CharExpr &spec) {
  ...
  case ParseKind::.cppess: specKind = IoSpecKind::.cppess; break;
  ...
  if (const std::optional<std::string> charConst{GetConstExpr<std::string>(
    ...
    if (specKind == IoSpecKind::.cppess) {
      flags_.set(Flag::Known.cppess);
      flags_.set(Flag::.cppessDirect, s == "DIRECT");
      flags_.set(Flag::.cppessStream, s == "STREAM");
    }
...
void IoChecker::Leave(const parser::OpenStmt &) {
  ...
  if (flags_.test(Flag::Known.cppess)) {
    CheckForRequiredSpecifier(flags_.test(Flag::.cppessDirect),
        "ACCESS='DIRECT'", IoSpecKind::Recl);  // 12.5.6.15
    CheckForProhibitedSpecifier(flags_.test(Flag::.cppessStream),
...
void IoChecker::CheckStringValue(IoSpecKind specKind, const std::string &value,
  ...
  static std::unordered_map<IoSpecKind, const std::set<std::string>> specValues{
      {IoSpecKind::.cppess, {"DIRECT", "SEQUENTIAL", "STREAM"}},
