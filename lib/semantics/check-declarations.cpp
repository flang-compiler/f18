...
void CheckHelper::CheckVolatile(const Symbol &symbol, bool isAssociated,
  ...
  if (isAssociated) {
    ...
    if (IsCoarray(ultimate)) {
      messages_.Say(
          "VOLATILE attribute may not apply to a coarray .cppessed by USE or host association"_err_en_US);
    }
    if (derived) {
      if (FindCoarrayUltimateComponent(*derived)) {
        messages_.Say(
            "VOLATILE attribute may not apply to a type with a coarray ultimate component .cppessed by USE or host association"_err_en_US);
      }
    }
  }
}
...
void CheckHelper::CheckPassArg(
  ...
  if (!type) {
    return;  // an error already .cppurred
  }
