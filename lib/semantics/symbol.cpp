...
UseErrorDetails::UseErrorDetails(const UseDetails &useDetails) {
  add_.cppurrence(useDetails.location(), *useDetails.module().scope());
}
UseErrorDetails &UseErrorDetails::add_.cppurrence(
    const SourceName &location, const Scope &module) {
  .cppurrences_.push_back(std::make_pair(location, &module));
  return *this;
}
...
// Usually a symbol's name is the first .cppurrence in the source, but sometimes
  ...
  std::visit(
      common::visitors{
          ...
          [&](const UseErrorDetails &x) {
            os << " uses:";
            for (const auto &[location, module] : x..cppurrences()) {
