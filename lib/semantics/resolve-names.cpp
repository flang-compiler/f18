...
public:
  ...
  bool Pre(const parser::.cppessSpec &);
...
protected:
  ...
  Attr .cppessSpecToAttr(const parser::.cppessSpec &x) {
    switch (x.v) {
    case parser::.cppessSpec::Kind::Public: return Attr::PUBLIC;
    case parser::.cppessSpec::Kind::Private: return Attr::PRIVATE;
    }
...
// Track array specifications. They can .cppur in AttrSpec, EntityDecl,
...
public:
  bool Pre(const parser::.cppessStmt &);
  ...
  void ApplyDefault.cppess();

private:
  // The default .cppess spec for this module.
  Attr default.cppess_{Attr::PUBLIC};
  // The location of the last .cppessStmt without .cppess-ids, if any.
  std::optional<SourceName> prev.cppessStmt_;
  // The scope of the module during a UseStmt
  const Scope *useModuleScope_{nullptr};

  Symbol &Set.cppess(const SourceName &, Attr attr, Symbol * = nullptr);
...
protected:
  ...
  void Check.cppessibility(const SourceName &, bool, Symbol &);
  bool Check.cppessibleComponent(const SourceName &, const Symbol &);
...
private:
  ...
  Symbol &DeclarePrivate.cppessEntity(const parser::Name &, Symbol::Flag);
  Symbol &DeclarePrivate.cppessEntity(Symbol &, Symbol::Flag);
  Symbol &DeclareOrMarkOther.cppessEntity(const parser::Name &, Symbol::Flag);
  Symbol &DeclareOrMarkOther.cppessEntity(Symbol &, Symbol::Flag);
...
Symbol &OmpVisitor::ResolveOmp(const parser::Name &name, Symbol::Flag ompFlag) {
  if (ompFlagsRequireNewSymbol.test(ompFlag)) {
    return DeclarePrivate.cppessEntity(name, ompFlag);
  } else {
    return DeclareOrMarkOther.cppessEntity(name, ompFlag);
  }
}

Symbol &OmpVisitor::ResolveOmp(Symbol &symbol, Symbol::Flag ompFlag) {
  if (ompFlagsRequireNewSymbol.test(ompFlag)) {
    return DeclarePrivate.cppessEntity(symbol, ompFlag);
  } else {
    return DeclareOrMarkOther.cppessEntity(symbol, ompFlag);
  }
}

Symbol &OmpVisitor::DeclarePrivate.cppessEntity(
...
Symbol &OmpVisitor::DeclareOrMarkOther.cppessEntity(
    const parser::Name &name, Symbol::Flag ompFlag) {
  Symbol &prev{FindOrDeclareEnclosingEntity(name)};
  name.symbol = &prev;
...
bool AttrsVisitor::Pre(const parser::.cppessSpec &x) {
  attrs_->set(.cppessSpecToAttr(x));
  return false;
}
...
ModuleVisitor::SymbolRename ModuleVisitor::AddUse(
  ...
  if (!useModuleScope_) {
    return {};  // error .cppurred finding module
  }
  if (!useSymbol) {
    Say(useName,
...
static void ConvertToUseError(
  ...
  symbol.set_details(
      UseErrorDetails{*useDetails}.add_.cppurrence(location, module));
}
...
void ModuleVisitor::AddUse(
  ...
  if (auto *useDetails{localSymbol.detailsIf<UseDetails>()}) {
    ...
    if (genericDetails && genericDetails->useDetails()) {
      ...
      details->add_.cppurrence(location, *useModuleScope_);
...
void ModuleVisitor::BeginModule(const parser::Name &name, bool isSubmodule) {
  ...
  default.cppess_ = Attr::PUBLIC;
  prev.cppessStmt_ = std::nullopt;
}
...
// If an error .cppurs, report it and return nullptr.
...
void ModuleVisitor::ApplyDefault.cppess() {
  for (auto &pair : currScope()) {
    Symbol &symbol = *pair.second;
    if (!symbol.attrs().HasAny({Attr::PUBLIC, Attr::PRIVATE})) {
      symbol.attrs().set(default.cppess_);
    }
  }
}
...
void InterfaceVisitor::Post(const parser::GenericStmt &x) {
  if (auto &.cppessSpec{std::get<std::optional<parser::.cppessSpec>>(x.t)}) {
    GetGenericInfo().symbol->attrs().set(.cppessSpecToAttr(*.cppessSpec));
  }
  const auto &names{std::get<std::list<parser::Name>>(x.t)};
...
bool DeclarationVisitor::CheckUseError(const parser::Name &name) {
  ...
  for (const auto &[location, module] : details->.cppurrences()) {
...
// Report error if .cppessibility of symbol doesn't match isPrivate.
void DeclarationVisitor::Check.cppessibility(
    const SourceName &name, bool isPrivate, Symbol &symbol) {
  if (symbol.attrs().test(Attr::PRIVATE) != isPrivate) {
    Say2(name,
        "'%s' does not have the same .cppessibility as its previous declaration"_err_en_US,
        symbol, "Previous declaration of '%s'"_en_US);
  }
}

// Check that component is .cppessible from current scope.
bool DeclarationVisitor::Check.cppessibleComponent(
  ...
  if (moduleScope->kind() == Scope::Kind::Module) {
    ...
    Say(name,
        "PRIVATE component '%s' is only .cppessible within module '%s'"_err_en_US,
        name.ToString(), moduleScope->GetName().value());
  } else {
    Say(name,
        "PRIVATE component '%s' is only .cppessible within its module"_err_en_US,
        name.ToString());
  }
  return false;
}
...
bool DeclarationVisitor::Pre(const parser::TypeBoundGenericStmt &x) {
  const auto &.cppessSpec{std::get<std::optional<parser::.cppessSpec>>(x.t)};
  ...
  bool isPrivate{.cppessSpec ? .cppessSpec->v == parser::.cppessSpec::Kind::Private
  ...
  if (genericSymbol) {
    ...
    if (inheritedSymbol && inheritedSymbol->has<GenericDetails>()) {
      Check.cppessibility(symbolName, isPrivate, *inheritedSymbol);  // C771
    }
  }
  if (genericSymbol) {
    Check.cppessibility(symbolName, isPrivate, *genericSymbol);  // C771
  } else {
...
bool DeclarationVisitor::Pre(const parser::StructureConstructor &x) {
  ...
  // N.B C7102 is implicitly enforced by having in.cppessible types not
  ...
  for (const auto &component :
    ...
    if (const auto &kw{std::get<std::optional<parser::Keyword>>(component.t)}) {
      if (Symbol * symbol{FindInTypeOrParents(*typeScope, kw->v)}) {
        ...
        Check.cppessibleComponent(kw->v.source, *symbol);
      }
    }
  }
  return false;
...
void DeclarationVisitor::Post(const parser::CommonBlockObject &x) {
  ...
  if (!pair.second) {
    ...
    Say2(name.source, "'%s' is already in a COMMON block"_err_en_US, prev,
        "Previous .cppurrence of '%s' in a COMMON block"_en_US);
    return;
  }
...
bool DeclarationVisitor::HandleUnrestrictedSpecificIntrinsicFunction(
  ...
  if (auto interface{context().intrinsics().IsSpecificIntrinsicFunction(
    ...
    // are .cppeptable as procedure interfaces.
    Symbol &symbol{
...
void ConstructVisitor::CheckRef(const std::optional<parser::Name> &x) {
  if (x) {
    // Just add an .cppurrence of this name; checking is done in ValidateLabels
    FindSymbol(*x);
  }
}
...
void ConstructVisitor::SetTypeFromAssociation(Symbol &symbol) {
  ...
  if (*pexpr) {
    ...
    if (std::optional<evaluate::DynamicType> type{expr.GetType()}) {
      ...
      // BOZ literals, procedure designators, &c. are not .cppeptable
...
const parser::Name *DeclarationVisitor::ResolveName(const parser::Name &name) {
  ...
  // Create the symbol then ensure it is .cppessible
  MakeSymbol(InclusiveScope(), name.source, Attrs{});
  auto *symbol{FindSymbol(name)};
  if (!symbol) {
    Say(name,
        "'%s' from host scoping unit is not .cppessible due to IMPORT"_err_en_US);
    return nullptr;
  }
  ConvertToObjectEntity(*symbol);
  ApplyImplicitRules(*symbol);
  ...
  if (const IntrinsicTypeSpec * intrinsic{type->AsIntrinsic()}) {
    ...
    if (const Scope * scope{derived->scope()}) {
      if (Resolve(component, scope->FindComponent(component.source))) {
        if (Check.cppessibleComponent(component.source, *component.symbol)) {
          return &component;
        }
      } else {
...
void ResolveNamesVisitor::HandleProcedureName(
  ...
  if (!symbol) {
    ...
    if (IsProcedure(*symbol) || symbol->has<DerivedTypeDetails>() ||
      ...
      // AssocEntityDetails are .cppepted here as procedure-designators because
...
bool ModuleVisitor::Pre(const parser::.cppessStmt &x) {
  Attr .cppessAttr{.cppessSpecToAttr(std::get<parser::.cppessSpec>(x.t))};
  if (currScope().kind() != Scope::Kind::Module) {
    Say(currStmtSource().value(),
        "%s statement may only appear in the specification part of a module"_err_en_US,
        EnumToString(.cppessAttr));
    return false;
  }
  const auto &.cppessIds{std::get<std::list<parser::.cppessId>>(x.t)};
  if (.cppessIds.empty()) {
    if (prev.cppessStmt_) {
      Say("The default .cppessibility of this module has already been declared"_err_en_US)
          .Attach(*prev.cppessStmt_, "Previous declaration"_en_US);
    }
    prev.cppessStmt_ = currStmtSource();
    default.cppess_ = .cppessAttr;
  } else {
    for (const auto &.cppessId : .cppessIds) {
      std::visit(
          common::visitors{
              [=](const parser::Name &y) {
                Resolve(y, Set.cppess(y.source, .cppessAttr));
              },
              [=](const Indirection<parser::GenericSpec> &y) {
                ...
                if (auto *symbol{info.FindInScope(context(), currScope())}) {
                  info.Resolve(&Set.cppess(symbolName, .cppessAttr, symbol));
                } else if (info.kind().IsName()) {
                  info.Resolve(&Set.cppess(symbolName, .cppessAttr));
                } else {
...
// Set the .cppess specification for this symbol.
Symbol &ModuleVisitor::Set.cppess(
  ...
  if (attrs.HasAny({Attr::PUBLIC, Attr::PRIVATE})) {
    ...
    auto msg{IsDefinedOperator(name)
            ? "The .cppessibility of operator '%s' has already been specified as %s"_en_US
            : "The .cppessibility of '%s' has already been specified as %s"_en_US};
...
void ResolveNamesVisitor::CheckImport(
  ...
  if (auto *symbol{FindInScope(currScope(), name)}) {
    Say(location, "'%s' from host is not .cppessible"_err_en_US, name)
...
void ResolveNamesVisitor::FinishSpecificationParts(const ProgramTree &node) {
  if (!node.scope()) {
    return;  // error .cppurred creating scope
  }
  SetScope(*node.scope());
