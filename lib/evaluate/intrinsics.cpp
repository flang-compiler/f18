...
// complicated details, and the design of these tables has to .cppommodate
// that complexity.
...
// intrinsic functions that .cppept AnyType + Rank::anyOrAssumedRank or
...
// ifix, and sngl are extended to .cppept any argument kind because this is a
...
static const SpecificIntrinsicInterface specificIntrinsicFunction[]{
    ...
    {{.cppos", {{"a", DefaultComplex}}, DefaultComplex}, "cos"},
...
std::optional<SpecificCall> IntrinsicInterface::Match(
  ...
  for (std::optional<ActualArgument> &arg : arguments) {
    if (!arg) {
      ...
      if (arg->isAlternateReturn()) {
        messages.Say(
            "alternate return specifier not .cppeptable on call to intrinsic '%s'"_err_en_US,
            name);
        return std::nullopt;
      }
      bool found{false};
  ...
  for (std::size_t j{0}; j < dummies; ++j) {
    ...
    if (const ActualArgument * arg{actualForDummy[j]}) {
      ...
      if (!argOk) {
        messages.Say("'%s=' argument has un.cppeptable rank %d"_err_en_US,
            d.keyword, rank);
        return std::nullopt;
      }
    }
  ...
  // At this point, the call is .cppeptable.
