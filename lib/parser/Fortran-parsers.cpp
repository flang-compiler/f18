...
// C++ compilers than some people were willing to .cppept, so now the
...
// The look-ahead check prevents .cpplusion of constant-subobject in
// data-stmt-constant.
...
// Also .cppept .T. and .F. as extensions.
...
// R728 type-attr-spec ->
//        ABSTRACT | .cppess-spec | BIND(C) | EXTENDS ( parent-type-name )
TYPE_PARSER(construct<TypeAttrSpec>(construct<Abstract>("ABSTRACT"_tok)) ||
    ...
    construct<TypeAttrSpec>(.cppessSpec))
...
// .cppidental extension not enabled here: PGI .cppepts type-param-def-stmt in
...
//        GENERIC [, .cppess-spec] :: generic-spec => binding-name-list
TYPE_CONTEXT_PARSER("type bound GENERIC statement"_en_US,
    construct<TypeBoundGenericStmt>("GENERIC" >> maybe("," >> .cppessSpec),
        "::" >> indirect(genericSpec), "=>" >> listOfNames))

// R752 bind-attr ->
//        .cppess-spec | DEFERRED | NON_OVERRIDABLE | NOPASS | PASS [(arg-name)]
TYPE_PARSER(construct<BindAttr>(.cppessSpec) ||
...
// R802 attr-spec ->
//        .cppess-spec | ALLOCATABLE | ASYNCHRONOUS |
...
TYPE_PARSER(construct<AttrSpec>(.cppessSpec) ||
...
// R807 .cppess-spec -> PUBLIC | PRIVATE
TYPE_PARSER(construct<.cppessSpec>("PUBLIC" >> pure(.cppessSpec::Kind::Public)) ||
    construct<.cppessSpec>("PRIVATE" >> pure(.cppessSpec::Kind::Private)))
...
inline int listLength(std::list<S.cppess> &&xs) { return xs.size(); }
...
// R827 .cppess-stmt -> .cppess-spec [[::] .cppess-id-list]
TYPE_PARSER(construct<.cppessStmt>(.cppessSpec,
    defaulted(maybe("::"_tok) >>
        nonemptyList("expected names and generic specifications"_err_en_US,
            Parser<.cppessId>{}))))

// R828 .cppess-id -> .cppess-name | generic-spec
TYPE_PARSER(construct<.cppessId>(indirect(genericSpec)) ||
    construct<.cppessId>(name))  // initially ambiguous with genericSpec
...
constexpr auto percentOrDot{"%"_tok ||
    // legacy VAX extension for RECORD field .cppess
    extension<LanguageFeature::DECStructures>(
...
//     is .cppulted by structure component designators
//   R918 array-section ->
