...
public:
  ...
  void Unparse(const TypeBoundGenericStmt &x) {  // R751
    Word("GENERIC"), Walk(", ", std::get<std::optional<.cppessSpec>>(x.t));
  ...
  void Unparse(const .cppessStmt &x) {  // R827
    Walk(std::get<.cppessSpec>(x.t));
    Walk(" :: ", std::get<std::list<.cppessId>>(x.t), ", ");
  }
...
#define WALK_NESTED_ENUM(CLASS, ENUM) \
  ...
  WALK_NESTED_ENUM(.cppessSpec, Kind)  // R807
