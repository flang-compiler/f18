...
// ambiguity is exacerbated by the extension that .cppepts a function
...
// may have been misplaced in the sequence of statements that are .cppeptable
...
//        .cppess-stmt | allocatable-stmt | asynchronous-stmt | bind-stmt |
...
TYPE_PARSER(first(
    construct<OtherSpecificationStmt>(indirect(Parser<.cppessStmt>{})),
...
TYPE_PARSER(construct<Only>(Parser<Rename>{}) ||
    ...
    construct<Only>(name))  // TODO: ambiguous, .cppepted by genericSpec
...
// R1510 generic-stmt ->
//         GENERIC [, .cppess-spec] :: generic-spec => specific-procedure-list
TYPE_PARSER(construct<GenericStmt>("GENERIC" >> maybe("," >> .cppessSpec),
...
//         .cppess-spec | proc-language-binding-spec | INTENT ( intent-spec ) |
//         OPTIONAL | POINTER | PROTECTED | SAVE
TYPE_PARSER(construct<ProcAttrSpec>(.cppessSpec) ||
...
TYPE_CONTEXT_PARSER("FUNCTION statement"_en_US,
    construct<FunctionStmt>(many(prefixSpec), "FUNCTION" >> name,
        ...
        extension<LanguageFeature::OmitFunctionDummies>(
            construct<FunctionStmt>(  // PGI & Intel .cppept "FUNCTION F"
