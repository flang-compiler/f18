...
std::optional<S.cppess> StartNewSubprogram::Parse(ParseState &state) {
...
std::optional<S.cppess> EnterNonlabelDoConstruct::Parse(ParseState &state) {
  if (auto *ustate{state.userState()}) {
...
std::optional<S.cppess> LeaveDoConstruct::Parse(ParseState &state) {
  if (auto ustate{state.userState()}) {
