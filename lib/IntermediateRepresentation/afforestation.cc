...
struct FortranIRLowering {
  ...
  FortranIRLowering(semantics::SemanticsContext &sc, bool debugLinearIR)
    : fir{new Program("program_name")}, semanticsContext{sc},
      debugLinearIR{debugLinearIR} {}
  ~FortranIRLowering() { CHECK(!builder); }
  ...
  Program *program() { return fir; }
  template<typename T>
  void ProcessRoutine(const T &here, const std::string &name) {
    CHECK(!fir->containsProcedure(name));
    auto *subp{fir->getOrInsertProcedure(name, nullptr, {})};
    builder = new IntermediateRepresentationBuilder(
        *CreateBlock(subp->getLastRegion()));
    AnalysisData ad;
    ControlFlowAnalyzer linearize{ops, ad};
    Walk(here, linearize);
    if (debugLinearIR) {
        ...
        common::visitors{
            ...
            [&](const common::Indirection<parser::InquireStmt> &statement) {
              std::visit(
                  common::visitors{
                      [&](const std::list<parser::InquireSpec> &specifiers) {
                        builder->CreateIOCall(InputOutputCallInquire,
            ...
            [&](const common::Indirection<parser::NullifyStmt> &statement) {
              builder->CreateNullify(&*statement);
            },
            ...
            [&](const common::Indirection<parser::ReadStmt> &statement) {
              builder->CreateIOCall(InputOutputCallRead,
            ...
            [&](const common::Indirection<parser::WriteStmt> &statement) {
              builder->CreateIOCall(InputOutputCallWrite,
            ...
            [&](const common::Indirection<parser::AssignStmt> &statement) {
              builder->CreateAssign(std::get<parser::Name>(statement->t).symbol,
                  blockMap
                      .find(
  ...
  void InitiateConstruct(const parser::AssociateStmt *associateStmt) {
    ...
    builder->CreateExpr(associateStmt);
  }
  ...
  void InitiateConstruct(const parser::ChangeTeamStmt *changeTeamStmt) {
    builder->CreateExpr(changeTeamStmt);
  }
  void InitiateConstruct(const parser::NonLabelDoStmt *nonlabelDoStmt) {
    // evaluate e1, e2 [, e3] ...
    builder->CreateExpr(nonlabelDoStmt);
  }
  void InitiateConstruct(const parser::IfThenStmt *ifThenStmt) {
    builder->CreateExpr(std::get<parser::ScalarLogicalExpr>(ifThenStmt->t)
                            .thing.thing->typedExpr.get());
  }
  void InitiateConstruct(const parser::WhereConstructStmt *whereConstructStmt) {
    builder->CreateExpr(std::get<parser::LogicalExpr>(whereConstructStmt->t)
                            .thing->typedExpr.get());
  }
  void InitiateConstruct(
      const parser::ForallConstructStmt *forallConstructStmt) {
    builder->CreateExpr(forallConstructStmt);
  }

  void ConstructIntermediateRepresentation(AnalysisData &ad) {
    for (auto iter{ops.begin()}, iend{ops.end()}; iter != iend; ++iter) {
      const auto &op{*iter};
      std::visit(
          common::visitors{
              [&](const LinearLabel &linearLabel) {
                auto *newBlock{CreateBlock(builder->GetCurrentRegion())};
                blockMap.insert({linearLabel.get(), newBlock});
                if (builder->GetInsertionPoint()) {
                  builder->CreateBranch(newBlock);
                }
              ...
              [&](const LinearBeginConstruct &linearConstruct) {
                ...
                std::visit(common::visitors{
                               [](const auto &) {},
                               [&](const LinearLabel &linearLabel) {
                                 blockMap.insert({linearLabel.get(),
  ...
  void EnterRegion(const parser::CharBlock &pos) {
    auto *region{builder->GetCurrentRegion()};
    auto *scope{semanticsContext.globalScope().FindScope(pos)};
  ...
  void ExitRegion() {
    builder->SetCurrentRegion(builder->GetCurrentRegion()->GetEnclosing());
  }
  void CheckInsertionPoint() {
    if (!builder->GetInsertionPoint()) {
      builder->SetInsertionPoint(CreateBlock(builder->GetCurrentRegion()));
    }
  }
  void AddOrQueueBranch(LinearLabelRef dest) {
    auto iter{blockMap.find(dest)};
    if (iter != blockMap.end()) {
      builder->CreateBranch(iter->second);
    } else {
      using namespace std::placeholders;
      arcsToAdd.emplace_back(std::bind(
          ...
          builder, builder->GetInsertionPoint(), dest, _1));
    }
  }
  void AddOrQueueCGoto(Expression *condition, LinearLabelRef trueBlock,
    ...
    if (trueIter != blockMap.end() && falseIter != blockMap.end()) {
      ...
      arcsToAdd.emplace_back(std::bind(
          ...
          builder, builder->GetInsertionPoint(), condition, trueBlock,
          falseBlock, _1));
    }
  }
  ...
  void AddOrQueueSwitch(const Evaluation &condition,
    ...
    auto defer{false};
    auto defaultIter{blockMap.find(defaultLabel)};
    typename SWITCHTYPE::ValueSuccPairListType cases;
    if (defaultIter == blockMap.end()) {
      defer = true;
    } else {
      CHECK(values.size() == labels.size());
      auto valiter{values.begin()};
      for (auto lab : labels) {
        auto labIter{blockMap.find(lab)};
        if (labIter == blockMap.end()) {
          defer = true;
          break;
        } else {
    ...
    if (defer) {
      ...
      arcsToAdd.emplace_back(std::bind(
          ...
          builder, builder->GetInsertionPoint(), condition, defaultLabel,
          values, labels, function, _1));
    } else {
      function(builder, condition, defaultIter->second, cases);
    }
  }
  ...
  void AddOrQueueIGoto(AnalysisData &ad, const semantics::Symbol *symbol,
    ...
    if (defer) {
      ...
      arcsToAdd.emplace_back(std::bind(
          ...
          builder, builder->GetInsertionPoint(), nullptr /*symbol*/, useLabels,
          _1));
    } else {
      builder->CreateIndirectBr(ConvertToVariable(symbol), blocks);
    }
  }
  void DrawRemainingArcs() {
    for (auto &arc : arcsToAdd) {
      arc(blockMap);
    }
  }
  ...
  IntermediateRepresentationBuilder *builder{nullptr};
  Program *fir;
  std::list<LinearOp> ops;
  std::list<Closure> arcsToAdd;
  LabelMapType blockMap;
